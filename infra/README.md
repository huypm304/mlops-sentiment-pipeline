...
Shared registry code lives in `apps/backend/registry/` and is bundled into Lambdas via `scripts/prepare_lambda_bundles.sh` (called from `scripts/build_audit_lambda.sh` before Terraform apply).
