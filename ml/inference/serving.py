...
Packaged inside model.tar.gz under ``code/inference.py`` with ``ml/inference/``.
...
def model_fn(model_dir: str) -> dict[str, Any]:
    ...
    _ensure_code_path()
    from ml.inference.inference import load_model
...
def predict_fn(text: str, bundle: dict[str, Any]) -> dict[str, Any]:
    _ensure_code_path()
    from ml.inference.inference import predict_one
