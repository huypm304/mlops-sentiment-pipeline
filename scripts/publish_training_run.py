...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish training run artifacts to S3 + DynamoDB")
    parser.add_argument("--run-id", default=None, help="Run ID (default: run-YYYYMMDD-HHMMSS)")
    parser.add_argument("--model-dir", type=Path, default=Path("model"), help="Training output dir")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Package staging dir and print summary only")
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        work_dir = ROOT / "artifacts" / "runs" / "training-runs" / run_id
