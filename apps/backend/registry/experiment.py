"""Experiment tracking — package training outputs and publish to S3 + DynamoDB."""
...
RUNS_WORK_DIR = REPO_ROOT / "artifacts" / "runs"
...
def publish_training_run(
    *,
    ...
    """Package artifacts, upload to S3, and register the run in DynamoDB."""
    if store is None or not getattr(store.config, "artifacts_bucket", ""):
        raise ValueError("RegistryStore with ARTIFACTS_BUCKET is required (no local-only mode)")

    model_dir = model_dir.resolve()
    work_dir = RUNS_WORK_DIR / "training-runs" / run_id
